...
const AllUserNotes = () => {
  ...
  const handlePageChange = (page) => {
    if (page < 1 || page > (allUserNotesPagination.totalPages || 1)) return
    getAllUserNotes({
      subject,
      questionType,
      companyType,
  ...
  return (
    <div className="flex-1 bg-gradient-to-br from-teal-50 via-slate-50 to-amber-50 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 p-4 sm:p-6">
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-slate-900 dark:text-white">All User Notes</h1>
        <p className="text-slate-600 dark:text-slate-400 mt-2">See verified notes submitted by users</p>
      </div>
        ...
        {allUserNoteLoading ? (
          ...
          <>
            <div className="p-6 space-y-4">
              {allUserNotes.map((item) => {
                const noteLink = item.pdfUrl || item.fileUrl || item.secure_url || item.pdfFile
                return (
                  <div key={item._id} className="border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 p-5">
                    <h3 className="font-semibold text-slate-900 dark:text-white text-lg">{item.title || item.subject || "Untitled note"}</h3>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                      By: {item?.writtenBy?.username || "User"} ({item?.writtenBy?.email || "N/A"})
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <span className="px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">{item.subject || "N/A"}</span>
                      <span className="px-3 py-1 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-700">{item.cource || "N/A"}</span>
                      <span className="px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">{item.company || "N/A"}</span>
                      <span className="px-3 py-1 rounded-full text-xs font-semibold bg-sky-100 text-sky-700">{item.location || "N/A"}</span>
                    </div>
                    {noteLink ? (
                      <a
                        href={noteLink}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-block mt-4 text-sm font-semibold text-teal-700 dark:text-teal-300 hover:underline"
                      >
                        Open PDF
                      </a>
                    ) : null}
            ...
            <div className="p-6 border-t border-slate-200 dark:border-slate-700 flex justify-center gap-2">
              <button
                onClick={() => handlePageChange((allUserNotesPagination.currentPage || 1) - 1)}
                disabled={(allUserNotesPagination.currentPage || 1) === 1}
              ...
              <span className="px-4 py-2 text-slate-900 dark:text-white">
                Page {allUserNotesPagination.currentPage || 1} of {allUserNotesPagination.totalPages || 1}
              </span>
              <button
                onClick={() => handlePageChange((allUserNotesPagination.currentPage || 1) + 1)}
                disabled={(allUserNotesPagination.currentPage || 1) >= (allUserNotesPagination.totalPages || 1)}
                className="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white rounded-lg disabled:opacity-50"
              >
                Next
              </button>
          ...
          <div className="p-8 text-center text-slate-500">
            <p>No user notes found for selected filters.</p>
          </div>
        )}
      </div>
    </div>
